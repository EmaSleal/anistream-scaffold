...
export function LandingPage({ series }: LandingPageProps) {
  const heroSeries = series.filter((s) => s.bannerUrl);

  return (
    <div className={styles.root}>
      {heroSeries.length > 0 && <LandingHero series={heroSeries.slice(0, 8)} />}

      <section className={styles.cardsSection}>
        <h2 className={styles.sectionTitle}>Popular Right Now</h2>
        <div className={styles.cardsRow}>
          {series.map((s) => (
            <LandingCard key={s.id} series={s} />
          ))}
        </div>
      </section>

      <section className={styles.cta}>
        <p className={styles.ctaText}>Ready to start watching?</p>
        <button
        ...
        <p className={styles.footnote}>No credit card required.</p>
      </section>
    </div>
  );
}

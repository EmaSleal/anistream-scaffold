...
class TestProbeSources:
    def test_jkanime_available(self):
        ...
        assert sources == [{"source": "jkanime", "available": True}]

    def test_jkanime_unavailable(self):
        config = {"fallback_slug": "slug-jk"}
        with patch("domain.download_sources.resolve_jkanime_stream", return_value=_JK_MISS):
            sources = probe_sources(config, 1)
        assert sources == [{"source": "jkanime", "available": False}]

    def test_available_false_when_resolver_raises(self):
        config = {"fallback_slug": "slug-jk"}
        with patch("domain.download_sources.resolve_jkanime_stream", side_effect=RuntimeError("fail")):
            sources = probe_sources(config, 1)
        assert sources == [{"source": "jkanime", "available": False}]

    def test_empty_when_no_fallback_slug(self):
        config = {"principal_slug": "slug-av1"}
        assert probe_sources(config, 1) == []
...
class TestResolveSource:
    ...
    def test_returns_none_for_animeav1(self):
        config = {"principal_slug": "slug-av1", "fallback_slug": "slug-jk"}
